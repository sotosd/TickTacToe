 ...
 * @author root
 */
public class JoinRandGameHandler implements HttpHandler {
    
    public JoinRandGameHandler(){
...
// Get the request method.
    ...
    private void processPostMethod(HttpExchange he) {
        try {
            ...
            if (cm.getType() != CommunicationType.enter_random_game) {
            ...
            if(MyStatics.games.size()!=0){
                for (int i = 0; i < MyStatics.games.size(); i++) {
                    if(MyStatics.games.get(i).getP1()!=null && MyStatics.games.get(i).getP2()==null  /*&& MyStatics.games.get(i).getStatus() == GameStatus.waiting_player_2_to_join*/) {
                        gameExists = true;
                        gameId=i;
                        break;
                    }
            ...
            if (!gameExists) {
                ...
                CommunicationMessage rcm = new CommunicationMessage();
                rcm.setType(CommunicationType.game_not_exist);
                bw.write(new Gson().toJson(rcm));
                bw.close();
                he.close();
